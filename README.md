...
*   **🎫 Sistema de Tickets:**
    ...
    *   Sistema de valoraciones con estrellas y comentarios opcionales (solo visible para Operadores).
...
*   **🛡️ Moderación:**
    ...
    *   Las sanciones desde el **panel web** (timeout, kick, ban) se ejecutan directamente en Discord vía API. Los warns se registran en el historial y se envía DM al usuario.
...
*   **🔍 Auditoría y Logs:**
    ...
    *   **Trazabilidad completa:** todas las acciones del bot (warn/timeout/kick/ban) y del panel web (config, encuestas, sugerencias, sanciones) quedan registradas en `audit_logs` con executor, target y detalles.
...
*   **📢 Sugerencias:**
    ...
    *   Cambio de estado restringido a **Admin+** (tanto desde bot como desde el panel web).
...
*   **🖥️ Panel Web (dashboard):**
    ...
    *   Gestión completa de tickets, sanciones, sugerencias, encuestas, departamentos y embeds.
    *   Configuración de canales y roles directamente desde el navegador.
    *   Mensajes de bienvenida/despedida configurables desde la sección "Mensajes Embed" del panel.
    *   Visualización de transcripts de tickets en HTML.
    *   Consola SQL integrada (solo superadmin) con autocompletado y semilla de datos de prueba.
    *   Registro de auditoría con filtros por acción, executor y fecha.
